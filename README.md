...
Только для мониторинга СУБД (с привилегиями роли `pg_monitor`) и выполнения коротких команд подключитесь к базе `postgres` под пользователем `psqlrc_user`, который был создан при инсталляции.

При наличии [BiHA](https://postgrespro.ru/docs/enterprise/current/biha-reference) так же можно подключиться к базе `biha_db` под пользователем `biha_replication_user`.
В этом случае даже не потребуется добавлять пароль пользователя `psqlrc_user` в файл `~/.pgpass`.

Т.о. возможно подключится к СУБД с минимальными правами под любым из двух указанных пользователей.
