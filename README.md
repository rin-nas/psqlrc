...
Только для мониторинга СУБД (используются привилегии роли `pg_monitor`) и выполнения коротких команд 
подключитесь к базе `postgres` под пользователем `psqlrc_user`, который был создан при инсталляции.

При наличии [BiHA](https://postgrespro.ru/docs/enterprise/current/biha-reference) 
так же можно подключиться к базе `biha_db` под пользователем `biha_replication_user`.
В этом случае даже не потребуется добавлять пароль пользователя `psqlrc_user` в файл `~/.pgpass`.

Таким образом возможно подключится к СУБД с минимальными правами под любым из двух указанных пользователей.
...
Поэтому в таблицах коротких команд перед «проблемными» значениями показывается знак предупреждения ⚠️. Конкретные условия см. в разделе с примерами.
...
Для этих действий нужны права суперпользователя.
...
Пересоздаётся полностью заново только схема `pro`, роль `psqlrc_role` и пользователь `psqlrc_user` остаются без изменений.   
...
Описание некоторых колонок таблицы: 
...
Условия отображения предупреждения ⚠️:
