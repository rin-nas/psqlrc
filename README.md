...
Используя всего один файл [`~/.psqlrc`](.psqlrc) можно значительно улучшить приглашение, 
...
# 2) сохраните пароль для пользователя postgres, при необходимости
nano ~/.pgpass  

# 3a) подключитесь локально по сокету
psql -v pro=1

# 3b) или подключитесь сразу к мастеру
psql -v pro=1 'postgresql://postgres@host-1,host-2,host-3,host-4/postgres?target_session_attrs=read-write&connect_timeout=3'

# 3c) только приглашение командной строки без информационных сообщений и проверок (с флагом -q)
psql -v pro=0 -q
```
