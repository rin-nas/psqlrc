...
begin atomic
    ...
    from pro.dblink(  -- в случае недоступности сетевого соединения dblink() возвратит ошибку
               ping.connection_str,
               format($sql$
                      select
...
comment on function pro.ping(connection_str text) is 'Get remote IP, latency and date-time difference between current and remote servers';
